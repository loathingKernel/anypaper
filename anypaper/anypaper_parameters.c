...
void anypaper_parameters_position_test(AnypaperParameters *parameters, gint rangex, gint rangey, gint positionx,
                                       gint positiony) {
    if (((positionx < 0) && (positionx >= 2 * rangex)) || ((positionx >= 0) && (positionx <= 2 * rangex)))
        parameters->positionx = positionx;
    if (((positiony < 0) && (positiony >= 2 * rangey)) || ((positiony >= 0) && (positiony <= 2 * rangey)))
        ...
        for (i = 0; i != j; i++) {
            ...
            if ((field[i][0] != '#') && (t != 0)) {
                ...
                if (!g_ascii_strcasecmp(subfield[0], "PositionX")) {
                    parameters->positionx = g_ascii_strtoll(subfield[1], NULL, 10);
                    //parameters->positionx=spi(subfield[1], 10);
                    par_counter++;
                }
                if (!g_ascii_strcasecmp(subfield[0], "PositionY")) {
                    parameters->positiony = g_ascii_strtoll(subfield[1], NULL, 10);
                ...
                if (!g_ascii_strcasecmp(subfield[0], "Width")) {
                    parameters->width = g_ascii_strtoll(subfield[1], NULL, 10);
                    //parameters->width=spi(subfield[1], 10);
                    par_counter++;
                }
                if (!g_ascii_strcasecmp(subfield[0], "Height")) {
                    parameters->height = g_ascii_strtoll(subfield[1], NULL, 10);
                ...
                if (!g_ascii_strcasecmp(subfield[0], "JPEGQuality")) {
                    parameters->jpegQuality = g_ascii_strtoll(subfield[1], NULL, 10);
                    //parameters->width=spi(subfield[1], 10);
                    par_counter++;
                }
                if (!g_ascii_strcasecmp(subfield[0], "PNGCompression")) {
                    parameters->pngCompression = g_ascii_strtoll(subfield[1], NULL, 10);
                    //parameters->width=spi(subfield[1], 10);
                    par_counter++;
                }
                if (!g_ascii_strcasecmp(subfield[0], "Command"))
                    parameters->command = g_strdup_printf("%s", subfield[1]);
                if (!g_ascii_strcasecmp(subfield[0], "DefaultFile"))
                    parameters->defaultfile = g_strdup_printf("%s", subfield[1]);
                if (!g_ascii_strcasecmp(subfield[0], "Interpolation"))
                    parameters->interpolation = option_interpolation(subfield[1]);
                g_strfreev(subfield);
            }
            t = 0;
        }
...
Width:                  %d\n\
Height:                 %d\n\
BackgroundColor:        %s\n", parameters->file, style, parameters->positionx, parameters->positiony,
                             parameters->scalex, parameters->scaley, parameters->width, parameters->height,
...
Interpolation:             %s\n\
JPEGQuality:               %d\n\
PNGCompression:            %d\n", parameters->command, parameters->defaultfile, interpolation, parameters->jpegQuality,
                             parameters->pngCompression);
