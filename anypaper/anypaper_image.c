...
gboolean anypaper_image_make(AnypaperImage *image, AnypaperParameters *parameters) {
    ...
    gdk_pixbuf_fill(tempbuf, (guint32) g_ascii_strtoll(&(parameters->background)[1], NULL, 16) * 256);
    ...
    if (md == 2) {
        ...
        while (x < (screenw)) {
            while (y < (screenh)) {
                gdk_pixbuf_composite(image->priv->base, tempbuf, x, y, w, h, x, y, sx, sy, parameters->interpolation,
                                     0xff);
                y = y + (h);
            }
            x = x + (w);
...
void anypaper_image_move(AnypaperImage *image, AnypaperParameters *parameters) {
    ...
    gdk_pixbuf_composite(image->priv->base, image->image, x, y, dx, dy, ox, oy, sx, sy, parameters->interpolation,
                         0xff);
    image->preview = gdk_pixbuf_scale_simple(image->image, (gdouble) (120 * screenw / screenh), (gdouble) (120),
