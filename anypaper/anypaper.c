...
gboolean fullscreen = FALSE, normal = FALSE, tile = FALSE, maximize = FALSE, scale = FALSE, custom = FALSE, set = FALSE,
        last = FALSE, no_set = FALSE;
gboolean info = FALSE;
gchar *commandline = NULL;
int x_position = -65536, y_position = -65536, height = -65536, width = -65536;
double x_scale = -1, y_scale = -1;
gchar *background_color = NULL, *lastwallpaperfile = NULL, *rcfile = NULL, **remaining_args = NULL, *wallpapersetterfile
                = NULL;
...
int main(int argc, char *argv[]) {
    ...
    if (wallpapersetterfile == NULL) wallpapersetterfile = g_strdup_printf(
                                         "%s/.anypaper/wallpapersetters", g_get_home_dir());
    if (!g_file_test(wallpapersetterfile, G_FILE_TEST_EXISTS)) {
        ...
        if (wallpapersetter->wallpapersetter) {
            ...
            window->parameters->command = g_strdup_printf("%s %s", (gchar *) wallpapersetter->wallpapersetter->data,
                                                          (gchar *) wallpapersetter->command->data);
        } else g_print(
    ...
    if (last == TRUE) {
        if (!g_file_test(parameters->defaultfile, G_FILE_TEST_EXISTS)) g_printerr("Default output file not found\n");
            ...
            if (g_spawn_command_line_sync(buffer, &std_out, &std_err, &exitStatus, NULL) == FALSE) g_printerr(
                "Invalid command\n");
            else {
                if (*std_out) {
                    g_print("Output:\n%s", std_out);
