...
static void set_image_common(AnypaperWindow *window) {
    ...
    if (!anypaper_image_make(window->image, window->parameters)) {
        dialog_error = gtk_message_dialog_new(
            GTK_WINDOW(window->priv->window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
...
void open_image_file_cb(GtkWidget *widget, AnypaperWindow *window) {
    ...
    dialog = gtk_file_chooser_dialog_new("Open File", GTK_WINDOW(window->priv->window), GTK_FILE_CHOOSER_ACTION_OPEN,
...
static void set_image_background_cb(GtkWidget *widget, AnypaperWindow *window) {
    ...
    window->parameters->background = g_strdup_printf("#%c%c%c%c%c%c", background_buffer[1], background_buffer[2],
                                                     background_buffer[5], background_buffer[6], background_buffer[9],
...
gboolean set_wallpaper_common(AnypaperWindow *window) {
    ...
    if (!g_file_test(window->parameters->file, G_FILE_TEST_EXISTS)) {
        dialog = gtk_message_dialog_new(GTK_WINDOW(window->priv->window), GTK_DIALOG_DESTROY_WITH_PARENT,
                                        GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "No such file exists");
        gtk_window_set_title(GTK_WINDOW(dialog), "Error");
        g_signal_connect_swapped(dialog, "response", G_CALLBACK (gtk_widget_destroy), dialog);
        gtk_widget_show(dialog);
        result = FALSE;
        ...
        if ((g_str_has_suffix(down_filename, ".jpg")) || (g_str_has_suffix(down_filename, ".jpeg"))) {
            if (test_command_exists(window->parameters->command) == FALSE) {
                dialog = gtk_message_dialog_new(
                    GTK_WINDOW(window->priv->window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR,
                    GTK_BUTTONS_CLOSE, "Command not found");
                gtk_window_set_title(GTK_WINDOW(dialog), "Error");
                g_signal_connect_swapped(dialog, "response", G_CALLBACK (gtk_widget_destroy), dialog);
                gtk_widget_show(dialog);
                result = FALSE;
            } else {
                gdk_pixbuf_save(window->image->image, window->parameters->defaultfile, "jpeg", NULL, "quality",
                                g_strdup_printf("%d", window->parameters->jpegQuality), NULL);
                buffer = g_strdup_printf("%s \"%s\"", window->parameters->command, window->parameters->defaultfile);
            ...
            if (test_command_exists(window->parameters->command) == FALSE) {
                ...
                gdk_pixbuf_save(window->image->image, window->parameters->defaultfile, "png", NULL, "compression",
                                g_strdup_printf("%d", window->parameters->pngCompression), NULL);
                buffer = g_strdup_printf("%s \"%s\"", window->parameters->command, window->parameters->defaultfile);
                g_spawn_command_line_async(buffer, NULL);
            }
        } else {
...
void save_default_file_cb(GtkWidget *widget, AnypaperWindow *window) {
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        ...
        if ((g_str_has_suffix(down_filename, ".jpg")) || (g_str_has_suffix(down_filename, ".jpeg")) || (
                g_str_has_suffix(down_filename, ".png"))) {
            g_free(window->parameters->defaultfile);
...
void save_file_as_cb(GtkWidget *widget, AnypaperWindow *window) {
    ...
    gtk_file_chooser_set_current_folder(
        GTK_FILE_CHOOSER(dialog), g_path_get_dirname(gtk_entry_get_text(GTK_ENTRY(window->priv->file_entry))));
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        ...
        if ((g_str_has_suffix(down_filename, ".jpg")) || (g_str_has_suffix(down_filename, ".jpeg"))) gdk_pixbuf_save(
            window->image->image, filename, "jpeg", NULL, "quality",
            g_strdup_printf("%d", window->parameters->jpegQuality), NULL);
        else if (g_str_has_suffix(down_filename, ".png")) gdk_pixbuf_save(
            window->image->image, filename, "png", NULL, "compression",
            g_strdup_printf("%d", window->parameters->pngCompression), NULL);
        else {
            dialog_error = gtk_message_dialog_new(
                GTK_WINDOW(window->priv->window), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
...
void load_rcfile_cb(GtkWidget *widget, AnypaperWindow *window) {
    ...
    if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
        ...
        gtk_combo_box_set_active(
            GTK_COMBO_BOX(window->priv->combo_interpolation), interpolation_int(window->parameters->interpolation));
        gtk_image_set_from_pixbuf(GTK_IMAGE(window->priv->preview), window->image->preview);
        unblock_callback(window);
    }
    gtk_widget_destroy(dialog);
...
void detect_popup_cb(GtkWidget *widget, AnypaperWindow *window) {
    ...
    if (window->wallpapersetter->wallpapersetter) {
        ...
        dialog = gtk_dialog_new_with_buttons("Detected wallpapersetters", GTK_WINDOW(window->priv->window),
        ...
        if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
            gchar *command;
            atual = g_list_nth(window->wallpapersetter->wallpapersetter,
        ...
        dialog = gtk_message_dialog_new(GTK_WINDOW(window->priv->window), GTK_DIALOG_DESTROY_WITH_PARENT,
                                        ...
                                        "No wallpapersetter found. A list of known wallpapersetter can be found in %s. You can install a wallpapersetter from that list, manually add a command to it or use directly a command of your preference",
                                        wallpapersetterfile);
...
void anypaper_window_create(AnypaperWindow *self) {
    ...
    gtk_combo_box_set_active(
        GTK_COMBO_BOX(priv->combo_interpolation), interpolation_int(self->parameters->interpolation));
